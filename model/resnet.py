...
class ResNet50(object):
    ...
    def __init__(self, config, n_classes):
        ...
        self.input_tensor = tf.keras.Input(
    shape=[None, self.image_size, self.image_size, self.n_channels],
    dtype=tf.float32,
)
    ...
    def train(self, n_epochs, debug=False):
        ...
        with tf.Session() as sess:
            ...
            for epoch in range(n_epochs):
                ...
                os.makedirs(f"training/epoch{epoch}", exist_ok=True)
    

                # Save the model
                save_path = saver.save(sess, "training/epoch0/model.ckpt".format(epoch))
    ...
    def load_pred(self, image_path):
        '''
    Creates Prediction Dataset
    '''

    # Calculating Batch Size (in this case, batch size will be 1)
        batch = 1

    # Parsing Image
        parse_fn = lambda f: _parse_image(f, self.n_channels, self.image_size)

    # Creates Dataset with a single image path
        predict_dataset = (tf.data.Dataset.from_tensor_slices(tf.constant([image_path]))
        .map(parse_fn, num_parallel_calls=4)
        .batch(batch)
    )
    ...
    # Pass Image into Model's Input
        self.images = predict_iterator.get_next()
    # Return image path to match it with Predictions
        return [image_path]
